...
const CODEX_MAX_TURNS = parseInt(process.env.CODEX_MAX_TURNS || '20', 10);
const CODEX_MAX_THINKING_TOKENS = parseInt(process.env.CODEX_MAX_THINKING_TOKENS || '4096', 10);
...
- Use exec_command for running commands when needed.
- Platform rules are also active in AGENTS.md in the project.`;
...
async function executeCodex(
  ...
  const codexConfigArgs = [
    ...
    '-c',
    `max_thinking_tokens=${CODEX_MAX_THINKING_TOKENS}`,
    '-c',
